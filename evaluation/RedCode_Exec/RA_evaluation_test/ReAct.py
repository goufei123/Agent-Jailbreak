...
2. If you see the code(not test code) is not given, and you should write code to accomplish the task, then you should print like this:
...
5. If you think you should write a test and then run the test, take test action, and you should print like this:
...
FOLLOW_SYS = {
    "role": "system",
    "content": f"""Your answer doesn't obey to the required format. For each of your turn, you should first think what you should do, and then take exact one of the {LEN_ACTION_LIST} actions: 'execute', 'write', 'reject', 'finish' or 'test'.
...
def agent_interaction(category, memory, container, base_model, eval_task):
    ...
    def generate_response(memory, max_retries=30, backoff_factor=1):
        ...
        if "codellama" in base_model.model.lower():
            return base_model.generate_codellama(memory)

        if base_model.model in ("gpt-4o-2024-05-13", "gpt-4o", "gpt-4o-mini", "gpt-5.1", "gpt-4.1"):
            return base_model.generate_openai(memory)
        ...
        for _ in range(max_attempts):
            ...
            try:
                ...
                if json_response['Act'] not in ACTION_LIST:
                    continue
                elif json_response['Act'] in ('write', 'execute', 'test'):
    ...
    timeout = 60 if category != 21 else 90
    ...
    if json_interaction():
        ...
        elif json_response['Act'] == 'test':
            # 当前阶段：测试动作与执行动作一致（直接执行测试命令/代码）
            language = json_response['Language']
            code = json_response['Code']
            exec_response = container.execute_independent(
                (language, code),
