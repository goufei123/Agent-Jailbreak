...
class BaseModel:
    def __init__(self, model, temperature, top_p, seed, max_tokens, dry_run):
        ...
        if model in ("gpt-4o-2024-05-13", "gpt-4o", "gpt-4o-mini", "gpt-5.1", "gpt-4.1"):
        ...
        if model == 'CodeLlama-13b-Instruct-hf':
            ...
            local_path = os.getenv(
                "CODELLAMA_13B_LOCAL_PATH",
                "/root/local_model/codellama/codellama-13b-instruct-hf/"
            )

            # tokenizer 一般不需要 trust_remote_code
            self.tokenizer = AutoTokenizer.from_pretrained(local_path)

            # 让 HF 自动把权重加载到当前可见 GPU（配合 CUDA_VISIBLE_DEVICES=1）
    ...
    def generate_deepseek(self, messages):
        inputs = self.tokenizer.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt").to(
            self.deepseek.device)
        # tokenizer.eos_token_id is the id of <|EOT|> token
        outputs = self.deepseek.generate(inputs, max_new_tokens=self.max_tokens, top_p=self.top_p,
    ...
    def generate_deepseek_v2(self, messages):
        inputs = self.tokenizer.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt").to(
            self.deepseek_v2.device)
        # tokenizer.eos_token_id is the id of <|EOT|> token
        outputs = self.deepseek_v2.generate(inputs, max_new_tokens=self.max_tokens, do_sample=False, top_p=self.top_p,
                                            num_return_sequences=1, eos_token_id=self.tokenizer.eos_token_id)
        ans = self.tokenizer.decode(outputs[0][len(inputs[0]):], skip_special_tokens=True)
        return ans
    ...
    def generate_codellama(self, messages):
        # 和 codeqwen / llama3 类似，先用 chat_template 拼成文本
        text = self.tokenizer.apply_chat_template(
            messages,
            tokenize=False,
            add_generation_prompt=True
        )
        model_inputs = self.tokenizer([text], return_tensors="pt").to(self.codellama.device)
        generated_ids = self.codellama.generate(
            model_inputs.input_ids,
            max_new_tokens=self.max_tokens,
            do_sample=False,
            top_p=self.top_p,
        )
        # 去掉前面的 prompt 部分，只保留新生成的
        generated_ids = [
    ...
    def execute_with_retries(self, cmd, retries=3, timeout=1 * 60):
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                logging.warning(
                    f"Attempt {attempt + 1}: Command execution timed out in docker container with cmd:\n```{cmd}```")
            except Exception as e:
